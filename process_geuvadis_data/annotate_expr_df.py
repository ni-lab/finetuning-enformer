...
def load_genes() -> dict[str, Gene]:
    ...
    for line in gzip.open(GENCODE_PATH, "rt"):
        ...
        attributes_dict = get_gtf_attributes_dict(attributes)
        gene_id = attributes_dict["gene_id"]
        gene_name = attributes_dict.get("gene_name", None)
...
def main():
    ...
    df = pd.read_csv(args.input_csv_path, index_col=0)  # [genes, samples]
    df["stable_id"] = df.index.str.split(".").str[0]
    df["gencode_v12_gene_name"] = df.index.map(lambda x: genes_by_id[x].gene_name)
    df["our_gene_name"] = df["stable_id"].map(
        lambda x: gene_id_to_our_gene_names.get(x, None)
    )
    df["EUR_eGene"] = df.index.map(lambda x: x in eur_egenes)
    df["YRI_eGene"] = df.index.map(lambda x: x in yri_egenes)
    df["top_EUR_eqtl_rsid"] = df.index.map(lambda x: top_eur_eqtl_rsids.get(x, None))
    df["top_YRI_eqtl_rsid"] = df.index.map(lambda x: top_yri_eqtl_rsids.get(x, None))
    df["top_EUR_eqtl_distance"] = df.index.map(
        lambda x: top_eur_eqtl_distances.get(x, None)
    )
    df["top_YRI_eqtl_distance"] = df.index.map(
        lambda x: top_yri_eqtl_distances.get(x, None)
    )

    print("Number of EUR eGenes:", df["EUR_eGene"].sum())
    print("Number of YRI eGenes:", df["YRI_eGene"].sum())

    # Assert that our_gene_name is not None for EUR eGenes
    assert df[df["EUR_eGene"]]["our_gene_name"].notnull().all()
