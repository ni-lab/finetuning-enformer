...
import torch
from datasets import EnformerDataset, PairwiseDataset, PairwiseMPRADataset
...
def parse_args():
    ...
    parser.add_argument("mpra_data_path", type=str)
...
def main():
    ...
    train_dl = CombinedLoader(
        [
            ...
            torch.utils.data.DataLoader(
                MPRA_train_ds, batch_size=args.batch_size * 8, shuffle=True
            ),
        ],
        mode="max_size_cycle",
    )
    ...
    val_dl = [
        ...
        torch.utils.data.DataLoader(
            MPRA_val_ds, batch_size=args.batch_size * 8, shuffle=False
        ),
    ]
    ...
    model = PairwiseWithOriginalDataJointTrainingAndPairwiseMPRAFloatPrecision(
        lr=args.lr,
        n_total_bins=pairwise_train_ds.get_total_n_bins(),
        num_cells=MPRA_train_ds.get_num_cells(),
        cell_names=MPRA_train_ds.get_cell_names(),
