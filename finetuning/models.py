...
class PairwiseWithOriginalDataJointTrainingAndEmbeddingLossFloatPrecision(
    L.LightningModule
):
    def __init__(
        self,
    ...
    def forward(
        ...
        if not return_base_predictions:
            ...
            assert embs.shape[1] == self.hparams.n_total_bins
            X = embs[:, self.center_start : self.center_end, :]
            X = self.attention_pool(X)  # (S * H, enformer_hidden_dim)
            Y = self.prediction_head(X)  # (S * H, 1)
            Y = rearrange(Y, "(S H) 1 -> S H", H=2)
            Y = Y.mean(dim=1)
        ...
        for i, dl_batch in enumerate(batch):
            ...
            if i == 0:  # this is the pairwise data
                ...
                if X.shape[-1] != 4:
                    ...
                    X = rearrange(X, "S H L NC -> (S H) L NC")  # (S * H, L, 4)
                Y_hat, embs = self(X, return_embeddings=True)
                Y_hat = Y_hat[: X1.shape[0]] - Y_hat[X1.shape[0] :]
                mse_loss = self.mse_loss(Y_hat, Y)
                self.log("train/pairwise_mse_loss", mse_loss)
                loss += mse_loss

                # also add embedding loss
                # embs is (S * H, n_total_bins, enformer_hidden_dim)
                embs = rearrange(embs, "(S H) L D -> S H L D", H=2)
                embs = embs.mean(dim=1)  # (S, L, D)
