...
from datasets import (PairwiseRegressionH5Dataset,
                      ...
                      PairwiseRegressionMalinoisMPRADataset)
...
def parse_args():
    ...
    parser.add_argument("malinois_data_path", type=str)
...
def main():
    ...
    malinois_MPRA_train_ds = PairwiseRegressionMalinoisMPRADataset(
        args.malinois_data_path,
        split="train",
        reverse_complement=True,
        shift_max=args.random_shift_max,
    )
    malinois_MPRA_val_ds = PairwiseRegressionMalinoisMPRADataset(
        args.malinois_data_path,
        split="val",
    ...
    val_dl = [
        ...
        torch.utils.data.DataLoader(malinois_MPRA_val_ds, batch_size=args.batch_size),
    ]
    ...
    checkpointing_cb = ModelCheckpoint(
        dirpath=ckpts_dir,
        filename="epoch={epoch}-step={step}-val_loss={val/pairwise_regression_loss/dataloader_idx_0:.4f}",
        monitor="val/pairwise_regression_loss/dataloader_idx_0",
        mode="min",
        save_top_k=-1,
    ...
    model = PairwiseRegressionWithMalinoisMPRAJointTrainingFloatPrecision(
        ...
        n_total_bins_malinois=malinois_MPRA_train_ds.get_total_n_bins(),
