...
cat "${TMP_DIR}/${NR}/gene_metadata.tsv" | awk 'NR>1' | while read PARAM; do
	...
	# Check if the output file exists; if not, skip to the next gene
	if [ ! -f "${TMP_DIR}/${NR}/${GNAME}.train.bed" ]; then
		echo "Skipping ${GNAME} due to missing genotype data."
		continue
	fi
...
echo "${MODEL}: ${SECONDS} seconds elapsed" >> "training_time_elapsed.txt"